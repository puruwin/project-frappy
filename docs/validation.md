...
- `npm run build`: 21 páginas generadas correctamente.
- `npm run check`: 0 errores, 0 advertencias y 0 avisos.
- Playwright: 64 pruebas superadas en escritorio y móvil.
- axe: ninguna infracción WCAG A/AA detectada en las 20 rutas públicas revisadas.
...
- Redirecciones: todos los destinos existen, usan 301 y no forman cadenas.
...
- Astro preview no ejecuta las reglas de Netlify. Los estados 301 deben verificarse de nuevo en el despliegue.
